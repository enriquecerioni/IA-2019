...
def calcularHijo(nodo, nodoActual):
    nMatriz=copy.deepcopy(nodo.matriz)
    ...
    for i in range(0,8):
        ...
        if(nMatriz[nodo.fila+1][i]==0):
            nMatriz[nodo.fila+1][i]=1
            if (cantCeldasVaciaProcimaFila(llenarCeldaOcupadas(nMatriz))==0):
                end=1
            nombre= str(nodo.fila+1) + str(i)
            hijos.append(Nodo(nombre,copy.deepcopy(nMatriz),nodoActual,nodo.fila+1,end))
...
def crearRecursivo(nodo,matriz,abierto,cerrado):
    if (nodo.padre):
        hijos= calcularHijo(nodo,nodoActual)
        for hijo in hijos:
            nodo.hijo.append(hijo)
            abierto.append(hijo)
            if(hijo.fila==7):
